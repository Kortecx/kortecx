...
function StepCard({ step, index, onRemove, onUpdate, onSwap }: {
  ...
  return (
    <div className="workflow-step" style={{ minWidth: 280, maxWidth: 340 }}>
      ...
      {/* Task description */}
      <textarea className="textarea" style={{ minHeight: 50, marginBottom: 8, fontSize: 12 }}
...
export default function WorkflowBuilderPage() {
  ...
  // Workflow ID (generated once for logging)
  const workflowIdRef = useRef(`wf-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    ...
    <div style={{ padding: 24, maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-secondary btn-sm"><FileText size={13} /> Templates</button>
          <button className="btn btn-secondary btn-sm" disabled={steps.length === 0}><Save size={13} /> Save</button>
      ...
      <div className="card" style={{ padding: 20, marginBottom: 16 }}>
        ...
        <input className="input" style={{ maxWidth: 480, borderColor: nameError ? 'var(--error)' : undefined }}
          ...
          onChange={e => { setWorkflowName(e.target.value); if (e.target.value.trim()) setNameError(false);
            wfLogger.logInteraction('name.changed', { name: e.target.value }); }}
          onBlur={() => { if (!workflowName.trim()) setNameError(true); }} />
        {nameError && <div style={{ fontSize: 11, color: 'var(--error)', marginTop: 4 }}>Workflow name is required</div>}
      </div>
