...
#[must_use]
pub fn handle_line(line: &str, client: &SlackClient) -> String {
...
fn handle(req: &Req, client: &SlackClient) -> String {
    let id = req.id;
    match req.method.as_str() {
        "initialize" => format!(
            r#"{{"jsonrpc":"2.0","id":{id},"result":{{"protocolVersion":"{PROTOCOL_VERSION}","capabilities":{{"tools":{{}}}},"serverInfo":{{"name":"kx-connector-slack","version":"1"}}}}}}"#
        ),
        "tools/list" => format!(
...
mod tests {
    ...
    fn initialize_advertises_protocol_and_name() {
        ...
        assert!(reply.contains("kx-connector-slack"));
    ...
    fn tools_list_exposes_the_four_tools() {
        ...
        for tool in ["post_message", "read_channel", "search", "list_channels"] {
