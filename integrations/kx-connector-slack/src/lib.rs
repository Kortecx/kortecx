...
//! `kx-connector-slack` — a bundled Slack MCP connector.
//!
...
//! an agent consume / generate / publish Slack:
//!   - `post_message`  — post a message to a channel (publish),
//!   - `read_channel`  — read recent messages from a channel (consume),
//!   - `search`        — search messages across the workspace (consume),
//!   - `list_channels` — list the workspace's channels (consume).
//!
//! ## Credential discipline (D81 — secret-by-reference)
//! The connector authenticates with a Slack **bot token** (`xoxb-…`) supplied
...
//! environment (the `KX_SLACK_CREDENTIAL` variable, a JSON object holding
//! `bot_token`). The connector reads it and calls the Slack Web API with an
//! `Authorization: Bearer <token>` header. The secret value is never placed in a
...
//! When `KX_SLACK_FAKE` is set the connector runs fully offline with deterministic
...
//! Slack bot token. See [`slack::SlackClient::from_env`].
//!
...
// `SlackClient` / `SlackError` types. Re-exported at the crate root for callers.
