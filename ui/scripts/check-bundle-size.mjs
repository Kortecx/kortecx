 ...
 *
 * Budget: 672 KiB raw (688,128 B — the value enforced below; keep this line in
 ...
 *   - 670 KiB → 672 KiB (branch point-in-time history): the eager SDK client gains
 *     listBranchVersions / restoreBranch, the `BranchVersion` / `RestoreResult` models,
 *     the regenerated descriptor for the four new messages, and `AppSummary.lifecycle`
 *     (the draft badge field, carried on the summary so ONE listApps paints every badge).
 *     Rides the eager `common.js` like every entry above. Measured A/B with the SDK
 *     rebuilt from main for the baseline (same protocol as the script-primitive entry):
 *     main 684,952 B → this branch 687,338 B (+2,386 B eager); bumped to the next KiB
...
const BUDGET = Number(process.env.KX_UI_EAGER_BUDGET_BYTES ?? 688_128);
