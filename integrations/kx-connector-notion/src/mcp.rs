...
#[must_use]
pub fn handle_line(line: &str, client: &NotionClient) -> String {
...
fn handle(req: &Req, client: &NotionClient) -> String {
    let id = req.id;
    match req.method.as_str() {
        "initialize" => format!(
            r#"{{"jsonrpc":"2.0","id":{id},"result":{{"protocolVersion":"{PROTOCOL_VERSION}","capabilities":{{"tools":{{}}}},"serverInfo":{{"name":"kx-connector-notion","version":"1"}}}}}}"#
        ),
        "tools/list" => format!(
...
mod tests {
    ...
    fn initialize_advertises_protocol_and_name() {
        ...
        assert!(reply.contains("kx-connector-notion"));
    ...
    fn tools_list_exposes_the_four_tools() {
        ...
        for tool in ["search", "read_page", "create_page", "append_block"] {
