...
All endpoints are served by the Python engine under the `/api/agents/engine/` prefix.

### List All Experts

```
GET /api/agents/engine/list
```
...
```
POST /api/agents/engine/{id}/update
```
...
```
GET /api/agents/engine/{id}/versions/{file}
```
...
```
POST /api/agents/engine/{id}/restore
```
...
Alternatively, create experts via the API or by directly adding a directory under `engine/agents/local/` with the required files and updating `_registry.json`.
